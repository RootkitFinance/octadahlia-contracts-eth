...
describe.only("OctaDahlia", async function () {
    ...
    let paired: Contract;
    let uniswap: { router: Contract; factory: Contract; };
    ...
    describe('setUp(IUniswapV2Pair _pair, address dev6, address dev9, address _mge, bool _dictator)', function () {
        it('should only initialize once', async function () {
            ...
            // await expect(octaDahlia.connect(owner).setUp(uniswap.factory.address, user1.address, user2.address, mge_address, false)).to.not.be.reverted
